...
for (let i = 0; i < lines.length; i++) {
    ...
    if (lowerLine.includes("nhiều lựa chọn") || lowerLine.includes("4 lựa chọn") || lowerLine.includes("chọn một phương án")) {
    ...
    if (/^(?:Câu|Bài)\s*(?:hỏi)?\s*(?:\d+)?\s*[:.]/i.test(lineTrimmed) || /^(?:Câu|Bài)\s*(?:hỏi)?\s*(?:\d+)?\s*\*/i.test(lineTrimmed)) {
        if (currentBlock.trim()) blocks.push({ text: currentBlock, typeContext: currentTypeContext });
        currentBlock = line + '\n';
    } else {
        // If the line is a header like "Phần 1:..." we shouldn't append it to the NEXT question if currentBlock is empty!
        // Wait, if currentBlock is empty, and we see "Phần 1:", we shouldn't make the question text "Phần 1:\n".
        // Let's just ignore headers or let them be part of the question. 
        // Actually, it's better to just append it, and later when cleaning the question text, it will be kept, which is fine, or we can clear currentBlock if we see a new header and it's mostly empty.
        currentBlock += line + '\n';
    }
}
if (currentBlock.trim()) blocks.push({ text: currentBlock, typeContext: currentTypeContext });
