...
for (let i = 0; i < lines.length; i++) {
    ...
    if (lowerLine.includes("nhiều lựa chọn") || lowerLine.includes("4 lựa chọn") || lowerLine.includes("chọn một phương án") || lowerLine.includes("phương án đúng")) {
    ...
    if (/^(?:Câu|Bài)\s*(?:hỏi)?\s*(?:\d+)?\s*[:.\-]/i.test(lineTrimmed)) {
        if (currentBlock.trim()) blocks.push({ text: currentBlock, typeContext: currentTypeContext });
        currentBlock = line + '\n';
    } else if (/^Phần\s+\d+/i.test(lineTrimmed)) {
        // If we hit a "Phần" boundary, we should also push the current block (if it's a question)
        if (currentBlock.trim()) blocks.push({ text: currentBlock, typeContext: currentTypeContext });
        currentBlock = ""; // We discard the "Phần" header text, we only needed it to set currentTypeContext!
    } else {
        currentBlock += line + '\n';
    }
}
