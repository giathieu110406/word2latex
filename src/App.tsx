...
export default function App() {
  ...
  const [generalNoticeHasPoll, setGeneralNoticeHasPoll] = useState<boolean>(false);
  const [generalNoticePollQuestion, setGeneralNoticePollQuestion] = useState<string>("");
  const [generalNoticePollOptions, setGeneralNoticePollOptions] = useState<string[]>([
    "Đồng ý / Hài lòng",
    "Không đồng ý / Cần cải thiện"
  ]);
  const [generalNoticeAllowComment, setGeneralNoticeAllowComment] = useState<boolean>(true);
  const [generalNoticeRequireResponse, setGeneralNoticeRequireResponse] = useState<boolean>(true);
  ...
  const [editingNoticeHasPoll, setEditingNoticeHasPoll] = useState<boolean>(false);
  const [editingNoticePollQuestion, setEditingNoticePollQuestion] = useState<string>("");
  const [editingNoticePollOptions, setEditingNoticePollOptions] = useState<string[]>([
    "Đồng ý / Hài lòng",
    "Không đồng ý / Cần cải thiện"
  ]);
  const [editingNoticeAllowComment, setEditingNoticeAllowComment] = useState<boolean>(true);
  const [editingNoticeRequireResponse, setEditingNoticeRequireResponse] = useState<boolean>(true);
  const [editingNoticeIsPollClosed, setEditingNoticeIsPollClosed] = useState<boolean>(false);
  const [isUpdatingGeneralNotice, setIsUpdatingGeneralNotice] = useState<boolean>(false);
  const [viewingPollStatsNotif, setViewingPollStatsNotif] = useState<any | null>(null);

  // User voting & feedback interaction state
  const [userSelectedPollOptions, setUserSelectedPollOptions] = useState<Record<string, number>>({});
  const [userPollComments, setUserPollComments] = useState<Record<string, string>>({});
  const [isSubmittingPollVote, setIsSubmittingPollVote] = useState<Record<string, boolean>>({});
  ...
  const [noticeCountdown, setNoticeCountdown] = useState<number>(0);
  const [poppedNoticeIds, setPoppedNoticeIds] = useState<string[]>([]);

  // Load and save read/popped notification state per user from localStorage
  useEffect(() => {
    if (!user) {
    ...
    const storedPopped = localStorage.getItem(`popped_notifs_${user.uid}`);
    if (storedRead) {
      try {
    ...
    if (storedPopped) {
      try {
        setPoppedNoticeIds(JSON.parse(storedPopped));
      } catch (e) {
        console.error(e);
      }
    } else {
  ...
  const getUserVoteForNotif = (notif: any, userUid?: string, uDoc?: any) => {
    ...
    if (notif?.votes && notif.votes[userUid]) {
      return notif.votes[userUid];
    }
    if (uDoc?.pollVotes && uDoc.pollVotes[notif?.id]) {
      return uDoc.pollVotes[notif.id];
    }
    try {
      const local = localStorage.getItem(`poll_votes_${userUid}`);
      if (local) {
        const parsed = JSON.parse(local);
        if (parsed && parsed[notif?.id]) {
          return parsed[notif.id];
        }
      }
    } catch (e) {}
    return null;
  ...
  const getAllVotesForNotif = (
    notif: any,
    usersList: any[] = [],
    currentUserUid?: string,
    currentUserDoc?: any,
  ) => {
    const mergedVotes: Record<string, any> = { ...(notif?.votes || {}) };

    if (Array.isArray(usersList)) {
      usersList.forEach((u) => {
        if (u.uid && u.pollVotes && u.pollVotes[notif?.id]) {
          if (!mergedVotes[u.uid]) {
            mergedVotes[u.uid] = {
              ...u.pollVotes[notif.id],
              userName: u.displayName || u.email?.split("@")[0] || "Thành viên",
              userEmail: u.email || "",
            };
          }
        }
    ...
    if (currentUserUid && currentUserDoc?.pollVotes && currentUserDoc.pollVotes[notif?.id]) {
      if (!mergedVotes[currentUserUid]) {
        mergedVotes[currentUserUid] = currentUserDoc.pollVotes[notif.id];
      }
    }

    if (currentUserUid) {
      try {
        const local = localStorage.getItem(`poll_votes_${currentUserUid}`);
        if (local) {
          const parsed = JSON.parse(local);
          if (parsed && parsed[notif?.id] && !mergedVotes[currentUserUid]) {
            mergedVotes[currentUserUid] = parsed[notif.id];
          }
        }
      } catch (e) {}
    }
  ...
  const isNotificationMandatoryPending = (n: any) => {
    if (!n || !n.hasPoll || !n.requireResponse || !user) return false;
    if (n.isPollClosed || n.pollClosed || n.pollStatus === "closed") return false;
    const userVote = getUserVoteForNotif(n, user.uid, userDoc);
    return !userVote;
  };

  // Filter out stopped/closed polls for non-admin users completely
  const visibleNotifications = React.useMemo(() => {
    const isAdmin = isAdminUser(user, userDoc);
    if (isAdmin) return notifications;
    return notifications.filter(
      (n) => !(n.hasPoll && (n.isPollClosed || n.pollClosed || n.pollStatus === "closed"))
    );
  }, [notifications, user, userDoc]);

  // Auto-popup modal when a new notification arrives that hasn't been popped yet OR if there is a mandatory poll pending
  useEffect(() => {
    if (!user || visibleNotifications.length === 0) return;

    // If there is ANY mandatory poll pending for this user, force open the notification modal
    if (!isAdminUser(user, userDoc)) {
      const hasMandatoryPending = visibleNotifications.some(isNotificationMandatoryPending);
    ...
    const unreadNotifs = visibleNotifications.filter(
      (n) => !readNotificationIds.includes(n.id)
    );
    const unpopped = unreadNotifs.filter((n) => !poppedNoticeIds.includes(n.id));

    if (unpopped.length > 0) {
      const newlyPoppedIds = unpopped.map((n) => n.id);
      setPoppedNoticeIds((prev) => {
        const updated = [...new Set([...prev, ...newlyPoppedIds])];
        localStorage.setItem(`popped_notifs_${user.uid}`, JSON.stringify(updated));
        return updated;
      });

      setIsNotificationsOpen(true);

      // Unless user is admin, enforce mandatory 10s countdown!
      if (!isAdminUser(user, userDoc)) {
  ...
  }, [visibleNotifications, user, userDoc, readNotificationIds, poppedNoticeIds]);

  // Countdown timer effect for closing notification modal
  useEffect(() => {
    let timer: any = null;
    if (isNotificationsOpen && noticeCountdown > 0) {
      timer = setInterval(() => {
  ...
  }, [isNotificationsOpen, noticeCountdown]);

  // Intercept Escape key when notifications have mandatory restrictions
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && isNotificationsOpen) {
        const hasPendingMandatoryPoll = visibleNotifications.some(isNotificationMandatoryPending);
        const cannotClose = !isAdminUser(user, userDoc) && (noticeCountdown > 0 || hasPendingMandatoryPoll);
        if (cannotClose) {
          e.preventDefault();
          e.stopPropagation();
          if (hasPendingMandatoryPoll) {
            triggerToast("⚠️ Bạn cần hoàn thành bình chọn / đóng góp ý kiến bắt buộc trước khi đóng thông báo!", false);
          } else if (noticeCountdown > 0) {
            triggerToast(`⚠️ Vui lòng xem thông báo trong ${noticeCountdown} giây nữa trước khi đóng.`, false);
          }
        }
      }
    };
  ...
  }, [isNotificationsOpen, visibleNotifications, user, userDoc, noticeCountdown]);
  ...
  useEffect(() => {
    if (isNotificationsOpen && user && visibleNotifications.length > 0) {
  ...
  }, [isNotificationsOpen, visibleNotifications, user]);
  ...
  const handleSendGeneralNotification = async () => {
    ...
    const cleanOptions = generalNoticePollOptions.map((o) => o.trim()).filter(Boolean);
    if (generalNoticeHasPoll && cleanOptions.length === 0 && !generalNoticeAllowComment) {
      triggerToast("Cuộc bình chọn cần ít nhất 1 phương án lựa chọn hoặc cho phép nhập ý kiến.", false);
      return;
    }
    ...
    try {
      ...
      await addDoc(collection(db, "notifications"), {
        ...
        senderName: "Trần Gia Thiều (Admin)",
        createdAt: new Date().toISOString(),
        hasPoll: !!generalNoticeHasPoll,
        pollQuestion: generalNoticePollQuestion.trim() || generalNoticeTitle.trim(),
        pollOptions: generalNoticeHasPoll ? (cleanOptions.length > 0 ? cleanOptions : ["Đồng ý", "Không đồng ý"]) : [],
        allowComment: generalNoticeHasPoll ? generalNoticeAllowComment : false,
        requireResponse: generalNoticeHasPoll ? generalNoticeRequireResponse : false,
        isPollClosed: false,
        votes: {},
      });
      ...
      setGeneralNoticeHasPoll(false);
      setGeneralNoticePollQuestion("");
      setGeneralNoticePollOptions(["Đồng ý / Hài lòng", "Không đồng ý / Cần cải thiện"]);
      setGeneralNoticeAllowComment(true);
      setGeneralNoticeRequireResponse(true);
    } catch (e) {
  ...
  const handleUpdateGeneralNotification = async () => {
    ...
    const cleanOptions = editingNoticePollOptions.map((o) => o.trim()).filter(Boolean);
    if (editingNoticeHasPoll && cleanOptions.length === 0 && !editingNoticeAllowComment) {
      triggerToast("Cuộc bình chọn cần ít nhất 1 phương án lựa chọn hoặc cho phép nhập ý kiến.", false);
      return;
    }
    ...
    try {
      ...
      await updateDoc(doc(db, "notifications", editingNotificationId), {
        ...
        targetEmail: targetEmail,
        hasPoll: !!editingNoticeHasPoll,
        pollQuestion: editingNoticePollQuestion.trim() || editingNoticeTitle.trim(),
        pollOptions: editingNoticeHasPoll ? (cleanOptions.length > 0 ? cleanOptions : ["Đồng ý", "Không đồng ý"]) : [],
        allowComment: editingNoticeHasPoll ? editingNoticeAllowComment : false,
        requireResponse: editingNoticeHasPoll ? editingNoticeRequireResponse : false,
        isPollClosed: editingNoticeHasPoll ? !!editingNoticeIsPollClosed : false,
      });
      ...
      setEditingNoticeHasPoll(false);
      setEditingNoticePollQuestion("");
      setEditingNoticePollOptions(["Đồng ý / Hài lòng", "Không đồng ý / Cần cải thiện"]);
      setEditingNoticeAllowComment(true);
      setEditingNoticeRequireResponse(true);
      setEditingNoticeIsPollClosed(false);
    } catch (e) {
  ...
  const handleTogglePollStatus = async (notifId: string, currentIsClosed: boolean) => {
    try {
      await updateDoc(doc(db, "notifications", notifId), {
        isPollClosed: !currentIsClosed,
        pollClosedAt: !currentIsClosed ? new Date().toISOString() : null,
      });
      triggerToast(
        !currentIsClosed
          ? "Đã ngưng cuộc bình chọn. Thông báo đã được ẩn đối với tất cả thành viên!"
          : "Đã mở lại cuộc bình chọn thành công!",
        true
      );
      if (viewingPollStatsNotif && viewingPollStatsNotif.id === notifId) {
        setViewingPollStatsNotif((prev: any) =>
          prev ? { ...prev, isPollClosed: !currentIsClosed } : null
        );
      }
    } catch (e) {
      console.error("Lỗi thay đổi trạng thái bình chọn:", e);
      triggerToast("Lỗi khi cập nhật trạng thái bình chọn.", false);
    }
  };

  const handleSubmitPollResponse = async (notifId: string, notif: any) => {
    if (!user) {
      triggerToast("Vui lòng đăng nhập để bình chọn.", false);
      return;
    }

    if (notif.isPollClosed || notif.pollClosed || notif.pollStatus === "closed") {
      triggerToast("Cuộc bình chọn này đã ngưng nhận phản hồi từ ban quản trị.", false);
      return;
    }

    const selectedIndex = userSelectedPollOptions[notifId];
    const commentText = (userPollComments[notifId] || "").trim();

    const hasOptions = Array.isArray(notif.pollOptions) && notif.pollOptions.length > 0;
    if (hasOptions && selectedIndex === undefined) {
      triggerToast("Vui lòng chọn 1 phương án bình chọn trước khi gửi!", false);
      return;
    }
    if (!hasOptions && notif.allowComment && !commentText) {
      triggerToast("Vui lòng nhập ý kiến đóng góp của bạn trước khi gửi!", false);
      return;
    }

    setIsSubmittingPollVote((prev) => ({ ...prev, [notifId]: true }));
    try {
      const optionText =
        hasOptions && selectedIndex !== undefined && notif.pollOptions[selectedIndex]
          ? notif.pollOptions[selectedIndex]
          : "";

      const voteData = {
        optionIndex: selectedIndex !== undefined ? selectedIndex : -1,
        optionText: optionText,
        comment: commentText,
        userEmail: user.email || userDoc?.email || "",
        userName: userDoc?.displayName || user.displayName || user.email?.split("@")[0] || "Thành viên",
        votedAt: new Date().toISOString(),
      };

      // 1. Immediately cache in localStorage for flawless instant UI state
      try {
        const stored = localStorage.getItem(`poll_votes_${user.uid}`);
        const parsed = stored ? JSON.parse(stored) : {};
        parsed[notifId] = voteData;
        localStorage.setItem(`poll_votes_${user.uid}`, JSON.stringify(parsed));
      } catch (e) {
        console.warn("Local storage cache error:", e);
      }

      // 2. Persist in user's own profile document in Firestore (Guaranteed write permissions)
      try {
        await updateDoc(doc(db, "users", user.uid), {
          [`pollVotes.${notifId}`]: voteData,
        });
      } catch (userDocErr) {
        console.warn("User doc vote update note:", userDocErr);
      }

      // 3. Attempt direct notification update (if permitted by rules)
      try {
        await updateDoc(doc(db, "notifications", notifId), {
          [`votes.${user.uid}`]: voteData,
        });
      } catch (notifErr) {
        console.info("Direct notification doc write skipped (vote is securely recorded in user record):", notifErr);
      }

      // 4. Update React state immediately
      setUserDoc((prev: any) =>
        prev
          ? { ...prev, pollVotes: { ...(prev.pollVotes || {}), [notifId]: voteData } }
          : prev,
      );
      ...
      setNotifications((prev) =>
        prev.map((n) =>
          n.id === notifId
            ? { ...n, votes: { ...(n.votes || {}), [user.uid]: voteData } }
            : n,
        ),
      );

      triggerToast("Cảm ơn bạn đã tham gia bình chọn / đóng góp ý kiến!");
      markNotificationAsRead(notifId);
    } catch (e: any) {
      console.error("Lỗi gửi bình chọn:", e);
      triggerToast("Lỗi khi gửi bình chọn: " + (e.message || "Vui lòng thử lại."), false);
    } finally {
      setIsSubmittingPollVote((prev) => ({ ...prev, [notifId]: false }));
    }
  };
  ...
  const injectMathML = (root: HTMLDivElement) => {
    root.querySelectorAll(".katex-custom-wrapper").forEach((wrapper) => {
      ...
      if (mathEl) {
        ...
        // Recompose KaTeX decomposed accents (<mover>) for Vietnamese / Latin characters into NFC
        const ACCENT_TO_COMBINING: Record<string, string> = {
          "\u00B4": "\u0301", // acute ´
          "\u02CA": "\u0301", // modifier letter acute
          "'": "\u0301",      // apostrophe as acute
          "\u0301": "\u0301", // combining acute
          "`": "\u0300",      // grave ` (\u0060)
          "\u02CB": "\u0300", // modifier letter grave
          "\u0300": "\u0300", // combining grave
          "^": "\u0302",      // circumflex
          "\u02C6": "\u0302",
          "\u0302": "\u0302",
        ...
        mathEl.querySelectorAll("mrow, math, semantics, mfrac, mtable, mtr, mtd").forEach((container) => {
          let prevMtext: Element | null = null;
          Array.from(container.children).forEach((child) => {
            if (child.tagName.toLowerCase() === "mtext") {
              if (prevMtext) {
                prevMtext.textContent = (prevMtext.textContent || "") + (child.textContent || "");
                child.remove();
              } else {
                prevMtext = child;
              }
    ...
    >
          ...
          <div className="sticky top-0 z-30 w-full bg-[#F8F9FD]/80 backdrop-blur-md px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                  ...
                  <button 
                    onClick={() => {
                      setIsNotificationsOpen(true);
                      if (!isAdminUser(user, userDoc) && unreadCount > 0) {
                        setNoticeCountdown(10);
                      } else {
                        setNoticeCountdown(0);
                      }
                    }} 
                    className="relative p-2.5 rounded-full bg-white border border-slate-200 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 transition-colors shadow-xs"
                  >
      ...
      <div className="max-w-full w-full px-4 sm:px-6 md:px-8 lg:px-10 py-2 md:py-4 flex-1 flex flex-col gap-4 md:gap-6 overflow-x-hidden">
        {(sidebarView === "members" || sidebarView === "feedbacks" || sidebarView === "notify" || sidebarView === "tracking") && isAdminUser(user, userDoc) && (
          ...
          >
            ...
            {sidebarView === "members" && (() => {
              ...
              const getUserDailyUsage = (u: any) => {
                const isReset = u.lastLatexResetDate !== currentTodayStr;
                if (isReset) return 0;
                const l = Number(u.latexCount) || 0;
                const e = Number(u.examCount) || 0;
                const p = Number(u.promptCount) || 0;
                return l + e + p;
              };
              ...
              const filteredUsers = allUsers
                ...
                .sort((a, b) => {
                  const usageA = getUserDailyUsage(a);
                  const usageB = getUserDailyUsage(b);
                  if (usageB !== usageA) {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 font-medium mt-0.5">
                        {editingNotificationId 
                          ? "Cập nhật lại tiêu đề, nội dung, đối tượng nhận hoặc cuộc bình chọn."
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          placeholder="Ví dụ: Cập nhật hệ thống v2.0 hoặc Khảo sát ý kiến..."
                      ...
                      <div className="p-3.5 bg-indigo-50/40 rounded-xl border border-indigo-100/80 space-y-3">
                        <div className="flex items-center justify-between">
                          <label className="flex items-center gap-2 cursor-pointer select-none">
                            <input
                              type="checkbox"
                              checked={editingNotificationId ? editingNoticeHasPoll : generalNoticeHasPoll}
                              onChange={(e) => {
                                const checked = e.target.checked;
                                if (editingNotificationId) {
                                  setEditingNoticeHasPoll(checked);
                                } else {
                                  setGeneralNoticeHasPoll(checked);
                                }
                              }}
                            ...
                            <span className="text-xs font-black text-indigo-900 flex items-center gap-1.5">
                              ...
                              Kèm cuộc bình chọn / Khảo sát
                            </span>
                          </label>
                          {(editingNotificationId ? editingNoticeHasPoll : generalNoticeHasPoll) && (
                        ...
                        {(editingNotificationId ? editingNoticeHasPoll : generalNoticeHasPoll) && (
                          <div className="space-y-3 pt-2 border-t border-indigo-100/70">
                            <div className="space-y-1">
                              <label className="text-[10px] font-bold text-slate-600 uppercase">Câu hỏi khảo sát / Bình chọn:</label>
                              <input
                                type="text"
                                value={editingNotificationId ? editingNoticePollQuestion : generalNoticePollQuestion}
                                onChange={(e) => {
                                  if (editingNotificationId) {
                                    setEditingNoticePollQuestion(e.target.value);
                                  } else {
                                    setGeneralNoticePollQuestion(e.target.value);
                                  }
                                }}
                                placeholder="Nếu để trống sẽ dùng tiêu đề thông báo..."
                                className="w-full bg-white border border-slate-200 focus:border-indigo-400 rounded-lg px-2.5 py-1.5 text-xs outline-none font-semibold text-slate-700 placeholder:text-slate-400"
                              />
                            </div>
                            ...
                            <div className="space-y-1.5">
                                  ...
                                  <div key={idx} className="flex items-center gap-1.5">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        if (editingNotificationId) {
                                          setEditingNoticePollOptions((prev) => {
                                            const updated = [...prev];
                                            updated[idx] = newVal;
                                            return updated;
                                          });
                                        } else {
                                          setGeneralNoticePollOptions((prev) => {
                                            const updated = [...prev];
                                            updated[idx] = newVal;
                                            return updated;
                                          });
                                      ...
                                      className="flex-1 bg-white border border-slate-200 focus:border-indigo-400 rounded-lg px-2.5 py-1 text-xs outline-none font-semibold text-slate-700"
                                    />
                                    {(editingNotificationId ? editingNoticePollOptions : generalNoticePollOptions).length > 1 && (
                                      <button
                                        type="button"
                                        onClick={() => {
                                          if (editingNotificationId) {
                                            setEditingNoticePollOptions((prev) => prev.filter((_, i) => i !== idx));
                                          } else {
                                            setGeneralNoticePollOptions((prev) => prev.filter((_, i) => i !== idx));
                                          }
                                        }}
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (editingNotificationId) {
                                    setEditingNoticePollOptions((prev) => [...prev, `Lựa chọn ${prev.length + 1}`]);
                                  } else {
                                    setGeneralNoticePollOptions((prev) => [...prev, `Lựa chọn ${prev.length + 1}`]);
                                  }
                                }}
                                className="w-full py-1 border border-dashed border-indigo-300 hover:border-indigo-500 bg-white hover:bg-indigo-50/50 text-indigo-700 rounded-lg text-[11px] font-bold transition-all flex items-center justify-center gap-1 cursor-pointer"
                              >
                            ...
                            <div className="space-y-2 pt-1">
                              <label className="flex items-center gap-2 cursor-pointer select-none">
                                <input
                                  type="checkbox"
                                  checked={editingNotificationId ? editingNoticeAllowComment : generalNoticeAllowComment}
                                  onChange={(e) => {
                                    const checked = e.target.checked;
                                    if (editingNotificationId) {
                                      setEditingNoticeAllowComment(checked);
                                    } else {
                                      setGeneralNoticeAllowComment(checked);
                                    }
                                  }}
                                  className="w-3.5 h-3.5 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 cursor-pointer"
                                />
                                <span className="text-[11px] font-bold text-slate-700">
                                  📝 Cho phép thành viên nhập nhận xét / ý kiến chi tiết
                                </span>
                              </label>
                              ...
                              <label className="flex items-center gap-2 cursor-pointer select-none">
                                <input
                                  type="checkbox"
                                  checked={editingNotificationId ? editingNoticeRequireResponse : generalNoticeRequireResponse}
                                  onChange={(e) => {
                                    const checked = e.target.checked;
                                    if (editingNotificationId) {
                                      setEditingNoticeRequireResponse(checked);
                                    } else {
                                      setGeneralNoticeRequireResponse(checked);
                                    }
                                  }}
                                ...
                                <span className="text-[11px] font-black text-rose-700 flex items-center gap-1">
                                  🔒 Bắt buộc bình chọn / góp ý trước khi được đóng thông báo
                                </span>
                              </label>

                              {editingNotificationId && editingNoticeHasPoll && (
                                <label className="flex items-center gap-2 cursor-pointer select-none bg-amber-50/80 p-2 rounded-lg border border-amber-200">
                                  <input
                                    type="checkbox"
                                    checked={editingNoticeIsPollClosed}
                                    onChange={(e) => setEditingNoticeIsPollClosed(e.target.checked)}
                                  ...
                                  <span className="text-[11px] font-black text-amber-900 flex items-center gap-1">
                                    ...
                                    Ngưng cuộc bình chọn này (Ẩn thông báo đối với tất cả thành viên)
                                  </span>
                                </label>
                              )}
                            </div>
                  ...
                  <div className="lg:col-span-7 bg-white rounded-xl border border-slate-200/80 p-5 flex flex-col shadow-2xs min-h-[400px]">
                    <div className="mb-4">
                      ...
                      <p className="text-[11px] text-slate-400 font-medium mt-0.5">Danh sách các tin tức và cuộc khảo sát đã phát đi trong hệ thống.</p>
                    </div>
                    ...
                    <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                      {allNotifications.map((n) => {
                        ...
                        const allNVotes = getAllVotesForNotif(n, allUsers, user?.uid, userDoc);
                        const voteCount = Object.keys(allNVotes).length;
                        const isPollStopped = !!(n.isPollClosed || n.pollClosed || n.pollStatus === "closed");
                            ...
                            <div className="space-y-1.5 flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-2 flex-wrap">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  ...
                                  {n.hasPoll && (
                                    <span
                                      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-black border ${
                                        isPollStopped
                                    ...
                                    >
                                      ...
                                      Bình chọn ({voteCount}) {isPollStopped ? "· Đã ngưng (Ẩn)" : "· Đang mở"}
                                    </span>
                                  )}
                                  {n.requireResponse && (
                                    <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-rose-50 text-rose-700 border border-rose-200">
                                      Bắt buộc
                                    </span>
                                  )}
                                ...
                                <div className="flex items-center gap-1 flex-wrap">
                                  {n.hasPoll && (
                                    <>
                                      <button
                                        type="button"
                                        onClick={() => handleTogglePollStatus(n.id, isPollStopped)}
                                        ...
                                        title={isPollStopped ? "Mở lại cuộc bình chọn và hiển thị lại cho thành viên" : "Ngưng cuộc bình chọn và ẩn thông báo đối với tất cả thành viên"}
                                      >
                                        {isPollStopped ? (
                                          <>
                                      ...
                                      <button
                                        onClick={() => setViewingPollStatsNotif(n)}
                                        className="text-[10px] text-purple-700 hover:text-purple-900 bg-purple-50 hover:bg-purple-100 border border-purple-200/60 px-2 py-0.5 rounded font-bold transition-all cursor-pointer flex items-center gap-1"
                                        title="Xem chi tiết kết quả bình chọn và ý kiến thành viên"
                                      >
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      setEditingNoticeHasPoll(!!n.hasPoll);
                                      setEditingNoticePollQuestion(n.pollQuestion || n.title || "");
                                      setEditingNoticePollOptions(
                                        Array.isArray(n.pollOptions) && n.pollOptions.length > 0
                                          ? n.pollOptions
                                          : ["Đồng ý / Hài lòng", "Không đồng ý / Cần cải thiện"]
                                      );
                                      setEditingNoticeAllowComment(n.allowComment !== undefined ? n.allowComment : true);
                                      setEditingNoticeRequireResponse(n.requireResponse !== undefined ? n.requireResponse : true);
                                      setEditingNoticeIsPollClosed(isPollStopped);
                              ...
                              {n.hasPoll && (
                                <div className="mt-1 p-2 bg-purple-50/60 rounded-lg border border-purple-100 text-[11px] text-purple-900 font-semibold space-y-1">
                                  <div className="flex items-center justify-between">
                                    <span className="font-extrabold flex items-center gap-1 text-purple-800">
                                      <Vote className="w-3 h-3" />
                                      {n.pollQuestion || "Khảo sát ý kiến"}
                                    </span>
                                  ...
                                  {Array.isArray(n.pollOptions) && n.pollOptions.length > 0 && (
                                    <div className="flex flex-wrap gap-1 pt-0.5">
                                      {n.pollOptions.map((opt: string, i: number) => {
                                        const optVoteCount = Object.values(allNVotes).filter(
                                          (v: any) => v.optionIndex === i || v.optionText === opt,
                                        ).length;
                                        return (
                                          <span
                                            key={i}
                                            className="px-1.5 py-0.5 bg-white border border-purple-200/80 rounded text-[9px] text-purple-800 font-medium"
                                          >
                                            {opt}: <b>{optVoteCount}</b>
                                          </span>
                                        );
                                      })}
                                    </div>
        ...
        <AnimatePresence>
          {isNotificationsOpen && (() => {
            const hasPendingMandatoryPoll = visibleNotifications.some(isNotificationMandatoryPending);
            const cannotCloseNotice = !isAdminUser(user, userDoc) && (noticeCountdown > 0 || hasPendingMandatoryPoll);
            ...
            return (
              <div
                ...
                onClick={(e) => {
                  if (e.target === e.currentTarget) {
                    if (noticeCountdown > 0 && !isAdminUser(user, userDoc)) {
                      triggerToast(`Vui lòng xem thông báo trong ${noticeCountdown} giây nữa trước khi đóng.`, false);
                      return;
                    }
                    if (hasPendingMandatoryPoll && !isAdminUser(user, userDoc)) {
                      triggerToast("Vui lòng hoàn thành bình chọn / đóng góp ý kiến bắt buộc trước khi đóng thông báo.", false);
                      return;
                    }
              ...
              >
                <motion.div
                  ...
                  animate={{ scale: 1, opacity: 1, y: 0 }}
                  exit={{ scale: 0.98, opacity: 0, y: 10 }}
                  className="bg-white rounded-2xl shadow-xl border border-slate-200/80 max-w-xl w-full overflow-hidden flex flex-col my-8 relative"
                >
                  {/* Header */}
                  <div className="bg-white px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                      <h3 className="font-extrabold text-sm md:text-base text-slate-800 flex items-center gap-2 flex-wrap">
                        <span>Thông Báo Hệ Thống & Khảo Sát</span>
                        {noticeCountdown > 0 && !isAdminUser(user, userDoc) && (
                          <span className="px-2 py-0.5 bg-amber-100 text-amber-800 font-extrabold text-[10px] rounded-full animate-pulse border border-amber-300">
                            Bắt buộc đọc ({noticeCountdown}s)
                          </span>
                        )}
                        {hasPendingMandatoryPoll && !isAdminUser(user, userDoc) && (
                          <span className="px-2 py-0.5 bg-rose-100 text-rose-800 font-extrabold text-[10px] rounded-full border border-rose-300 flex items-center gap-1">
                            <Vote className="w-2.5 h-2.5" />
                            Cần bình chọn
                          </span>
                        )}
                      ...
                      <p className="text-[10px] text-slate-400 font-semibold mt-0.5">
                        Cập nhật quan trọng và ý kiến đóng góp từ Ban Quản trị
                      </p>
                    </div>
                    {cannotCloseNotice ? (
                      <div className="px-2.5 py-1 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-[11px] font-black flex items-center gap-1.5 shadow-2xs">
                        {noticeCountdown > 0 ? (
                          <>
                            <span className="w-2 h-2 rounded-full bg-amber-500 animate-ping shrink-0" />
                            <span>{noticeCountdown}s</span>
                          </>
                        ) : (
                          <>
                            <Lock className="w-3 h-3 text-rose-600" />
                            <span className="text-rose-700">Khảo sát</span>
                          </>
                        )}
                      </div>
                    ) : (
                  ...
                  {noticeCountdown > 0 && !isAdminUser(user, userDoc) && (
                    <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white px-6 py-2.5 flex items-center justify-between text-xs font-bold shadow-xs">
                      <span className="flex items-center gap-2">
                        ⚠️ Thông báo mới từ Ban Quản trị! Bạn không thể đóng màn hình này trong 10 giây.
                      </span>
                      <span className="bg-black/20 px-2 py-0.5 rounded text-[11px] font-black shrink-0">
                        Còn {noticeCountdown}s
                      </span>
                    </div>
                  )}

                  {hasPendingMandatoryPoll && !isAdminUser(user, userDoc) && noticeCountdown === 0 && (
                    <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 py-2.5 flex items-center justify-between text-xs font-bold shadow-xs">
                      <span className="flex items-center gap-2">
                        🗳️ Thông báo có cuộc bình chọn bắt buộc! Vui lòng chọn và gửi ý kiến để tiếp tục sử dụng web.
                      </span>
                    </div>
                  )}
                  ...
                  <div className="p-6 overflow-y-auto max-h-[480px] space-y-4 bg-slate-50/30">
                    {visibleNotifications.length === 0 ? (
                      ...
                      visibleNotifications.map((notif) => {
                        const allNotifVotes = getAllVotesForNotif(notif, allUsers, user?.uid, userDoc);
                        const myVote = getUserVoteForNotif(notif, user?.uid, userDoc);
                        const hasVoted = !!myVote;
                        const isMandatoryPending = notif.hasPoll && notif.requireResponse && !hasVoted;

                        return (
                          <div
                            key={notif.id}
                            className={`bg-white hover:bg-slate-50/40 border rounded-xl p-4 flex flex-col justify-between transition-all shadow-2xs ${
                              isMandatoryPending ? "border-purple-300 ring-2 ring-purple-100" : "border-slate-200/60"
                            }`}
                          >
                            <div>
                              ...
                              {/* Interactive Poll / Survey Component */}
                              {notif.hasPoll && (
                                <div className="mt-3.5 p-3.5 bg-purple-50/70 border border-purple-200/80 rounded-xl space-y-3">
                                  <div className="flex items-center justify-between gap-2 flex-wrap">
                                    <h5 className="text-xs font-black text-purple-950 flex items-center gap-1.5">
                                      <Vote className="w-4 h-4 text-purple-600 shrink-0" />
                                      <span>{notif.pollQuestion || notif.title}</span>
                                    </h5>
                                    <div className="flex items-center gap-1">
                                      {(notif.isPollClosed || notif.pollClosed || notif.pollStatus === "closed") && (
                                        <span className="px-2 py-0.5 bg-rose-100 text-rose-800 font-extrabold text-[9px] rounded-md shrink-0 border border-rose-200">
                                          Đã ngưng bình chọn
                                        </span>
                                      )}
                                      {notif.requireResponse && !hasVoted && !(notif.isPollClosed || notif.pollClosed || notif.pollStatus === "closed") && (
                                        <span className="px-2 py-0.5 bg-rose-100 text-rose-700 font-extrabold text-[9px] rounded-md shrink-0 border border-rose-200">
                                          Bắt buộc phản hồi
                                        </span>
                                      )}
                                    </div>
                                  ...
                                  {/* If User has voted: Show vote status + breakdown */}
                                  {hasVoted && myVote ? (
                                    <div className="space-y-2.5 pt-1">
                                      <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1.5 rounded-lg border border-emerald-200">
                                        <Check className="w-3.5 h-3.5 shrink-0 text-emerald-600" />
                                        <span>
                                          Bạn đã bình chọn: <strong className="font-black text-emerald-900">{myVote.optionText || "Đã ghi nhận"}</strong>
                                        </span>
                                      </div>

                                      {myVote.comment && (
                                        <p className="text-[11px] text-slate-600 bg-white p-2.5 rounded-lg border border-slate-200 font-medium italic">
                                          💬 Ý kiến của bạn: "{myVote.comment}"
                                        </p>
                                      )}
                                      ...
                                      <div className="space-y-2 pt-1 border-t border-purple-100">
                                        <div className="flex items-center justify-between text-[11px] font-bold text-purple-900">
                                          ...
                                          <span>{Object.keys(allNotifVotes).length} lượt phản hồi</span>
                                        </div>

                                        <div className="space-y-2">
                                          {Array.isArray(notif.pollOptions) &&
                                            notif.pollOptions.map((opt: string, optIdx: number) => {
                                              const totalVotes = Object.keys(allNotifVotes).length;
                                              const count = Object.values(allNotifVotes).filter(
                                                (v: any) => v.optionIndex === optIdx || v.optionText === opt
                                              ).length;
                                              const pct = totalVotes > 0 ? Math.round((count / totalVotes) * 100) : 0;
                                              const isMyChoice =
                                                myVote.optionIndex === optIdx ||
                                                myVote.optionText === opt;

                                              return (
                                                <div key={optIdx} className="space-y-1">
                                                  <div className="flex justify-between items-center text-[11px] font-semibold text-slate-700">
                                                    <span className="flex items-center gap-1">
                                                      {isMyChoice && <Check className="w-3 h-3 text-purple-600" />}
                                                      <span className={isMyChoice ? "font-black text-purple-900" : ""}>
                                                        {opt}
                                                      </span>
                                                    </span>
                                                  ...
                                                  <div className="h-2 w-full bg-purple-100/80 rounded-full overflow-hidden">
                                                    <div
                                                      className={`h-full rounded-full transition-all duration-500 ${
                                                        isMyChoice ? "bg-purple-600" : "bg-purple-400"
                                                      }`}
                                                      style={{ width: `${pct}%` }}
                                                    />
                                                  </div>
                                    ...
                                    <div className="space-y-3 pt-1">
                                      <div className="space-y-1.5">
                                        ...
                                        <div className="grid grid-cols-1 gap-1.5">
                                          {Array.isArray(notif.pollOptions) &&
                                            notif.pollOptions.map((opt: string, optIdx: number) => {
                                              const isSelected = userSelectedPollOptions[notif.id] === optIdx;
                                              return (
                                                <button
                                                  type="button"
                                                  key={optIdx}
                                                  onClick={() => {
                                                    setUserSelectedPollOptions((prev) => ({
                                                      ...prev,
                                                      [notif.id]: optIdx,
                                                    }));
                                                  }}
                                                  className={`p-2.5 rounded-xl border text-left text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
                                                    isSelected
                                                      ...
                                                      : "bg-white hover:bg-purple-50/50 text-slate-700 border-purple-200/80"
                                                  }`}
                                                >
                                                  <span
                                                    className={`w-4 h-4 rounded-full border flex items-center justify-center text-[10px] shrink-0 ${
                                                      isSelected
                                                        ? "border-white bg-white text-purple-600 font-black"
                                      ...
                                      {notif.allowComment && (
                                        <div className="space-y-1">
                                          ...
                                          <textarea
                                            rows={2}
                                            value={userPollComments[notif.id] || ""}
                                            onChange={(e) => {
                                              const val = e.target.value;
                                              setUserPollComments((prev) => ({ ...prev, [notif.id]: val }));
                                            }}
                                            placeholder="Nhập suy nghĩ, đề xuất hoặc phản hồi của bạn..."
                                            className="w-full bg-white border border-purple-200 focus:border-purple-500 rounded-xl p-2 text-xs outline-none font-semibold text-slate-700 placeholder:text-slate-400"
                                          />
                                        </div>
                                      )}
                                      ...
                                      <button
                                        type="button"
                                        disabled={
                                          isSubmittingPollVote[notif.id] ||
                                          (Array.isArray(notif.pollOptions) && notif.pollOptions.length > 0 && userSelectedPollOptions[notif.id] === undefined) ||
                                          ((!Array.isArray(notif.pollOptions) || notif.pollOptions.length === 0) && notif.allowComment && !(userPollComments[notif.id] || "").trim())
                                        }
                                        onClick={() => handleSubmitPollResponse(notif.id, notif)}
                                        className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-purple-300 text-white py-2.5 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
                                      >
                                        {isSubmittingPollVote[notif.id] ? (
                                          <>
                                            <Loader2 className="w-3.5 h-3.5 animate-spin" />
                                            <span>Đang gửi bình chọn...</span>
                                          </>
                                        ) : (
                                          <>
                                            <Vote className="w-3.5 h-3.5" />
                                            <span>Gửi bình chọn / Ý kiến</span>
                                          </>
                                        )}
                                      </button>
                                    </div>
                            ...
                            <div className="mt-3.5 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px]">
                              ...
                              {notif.hasPoll && (
                                <div>
                                  {hasVoted ? (
                                    <span className="text-emerald-700 text-[10px] font-bold flex items-center gap-1 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                                      <Check className="w-2.5 h-2.5" />
                                      <span>Đã hoàn thành phản hồi</span>
                                    </span>
                                  ) : notif.requireResponse ? (
                                    <span className="text-rose-700 text-[10px] font-bold flex items-center gap-1 bg-rose-50 px-2 py-0.5 rounded border border-rose-200">
                                      <Lock className="w-2.5 h-2.5" />
                                      <span>Bắt buộc bình chọn</span>
                                    </span>
                                  ) : null}
                                </div>
                              )}
                  ...
                  <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                    {noticeCountdown > 0 && !isAdminUser(user, userDoc) ? (
                      <span className="text-[11px] font-bold text-amber-700 flex items-center gap-1">
                        🔒 Vui lòng xem thông báo trước khi tiếp tục (Còn {noticeCountdown}s)
                      </span>
                    ) : hasPendingMandatoryPoll && !isAdminUser(user, userDoc) ? (
                      <span className="text-[11px] font-bold text-rose-700 flex items-center gap-1">
                        🔒 Vui lòng hoàn thành bình chọn bắt buộc để tiếp tục
                      </span>
                    ) : (
                      <span className="text-[11px] font-semibold text-slate-400">
                        {unreadCount === 0 ? "Đã đọc tất cả thông báo" : `${unreadCount} thông báo chưa đọc`}
                      </span>
                    )}

                    <button
                      type="button"
                      disabled={cannotCloseNotice}
                      onClick={() => {
                        if (cannotCloseNotice) return;
                        setIsNotificationsOpen(false);
                      }}
                      className={`px-5 py-2 rounded-xl text-xs font-extrabold transition-all duration-200 shadow-sm flex items-center gap-2 ${
                        cannotCloseNotice
                    ...
                    >
                      {noticeCountdown > 0 && !isAdminUser(user, userDoc) ? (
                        <>
                          <span className="w-3.5 h-3.5 border-2 border-slate-400 border-t-amber-600 rounded-full animate-spin shrink-0"></span>
                          <span>Chờ {noticeCountdown}s để đóng</span>
                        </>
                      ) : hasPendingMandatoryPoll && !isAdminUser(user, userDoc) ? (
                        <>
                          <Lock className="w-3.5 h-3.5 text-rose-500" />
                          <span>Chưa thể đóng</span>
                        </>
                      ) : (
        ...
        {/* Poll Stats Modal for Admin */}
        <AnimatePresence>
          {viewingPollStatsNotif && (() => {
            const statsVotes = getAllVotesForNotif(viewingPollStatsNotif, allUsers, user?.uid, userDoc);
            const totalVotes = Object.keys(statsVotes).length;

            return (
              <div
                className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-60 overflow-y-auto"
                onClick={(e) => {
                  if (e.target === e.currentTarget) setViewingPollStatsNotif(null);
                }}
              >
                <motion.div
                  ...
                  className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-2xl w-full overflow-hidden flex flex-col my-8 max-h-[90vh]"
                >
                  <div className="bg-gradient-to-r from-purple-700 to-indigo-700 px-6 py-4 text-white flex justify-between items-center">
                    <div>
                      <h3 className="font-extrabold text-sm md:text-base flex items-center gap-2">
                        <BarChart3 className="w-4 h-4" />
                        <span>Thống Kê Kết Quả Bình Chọn</span>
                      </h3>
                      <p className="text-[10px] text-purple-200 font-semibold mt-0.5">
                        {viewingPollStatsNotif.title}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setViewingPollStatsNotif(null)}
                      className="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-white transition-colors cursor-pointer text-xs font-bold"
                    >
                      ✕
                    </button>
                  </div>
                  ...
                  <div className="p-6 overflow-y-auto space-y-5 flex-1">
                    {/* Question banner */}
                    <div className="p-3.5 bg-purple-50 rounded-xl border border-purple-100 space-y-2">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <span className="text-[10px] font-bold text-purple-600 uppercase tracking-wider block">Câu hỏi khảo sát:</span>
                        {isAdminUser(user, userDoc) && (
                          <button
                            type="button"
                            onClick={() => handleTogglePollStatus(viewingPollStatsNotif.id, !!(viewingPollStatsNotif.isPollClosed || viewingPollStatsNotif.pollClosed || viewingPollStatsNotif.pollStatus === "closed"))}
                            className={`text-[11px] px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer flex items-center gap-1.5 border shadow-2xs ${
                              (viewingPollStatsNotif.isPollClosed || viewingPollStatsNotif.pollClosed || viewingPollStatsNotif.pollStatus === "closed")
                          ...
                          >
                            {(viewingPollStatsNotif.isPollClosed || viewingPollStatsNotif.pollClosed || viewingPollStatsNotif.pollStatus === "closed") ? (
                              <>
                                ...
                                Mở lại cuộc bình chọn
                              </>
                            ) : (
                              <>
                                <PauseCircle className="w-3.5 h-3.5" />
                                Ngưng bình chọn (Ẩn với user)
                              </>
                            )}
                          </button>
                        )}
                      ...
                      <p className="text-sm font-black text-purple-950 mt-0.5">
                        {viewingPollStatsNotif.pollQuestion || viewingPollStatsNotif.title}
                      </p>
                      <div className="flex items-center gap-3 mt-2 text-[11px] font-bold text-slate-500 flex-wrap">
                        <span>Tổng số lượt tham gia: <strong className="text-purple-700 font-black">{totalVotes}</strong></span>
                        <span>•</span>
                        <span>Bắt buộc tham gia: <strong>{viewingPollStatsNotif.requireResponse ? "Có" : "Không"}</strong></span>
                        <span>•</span>
                        <span>
                          Trạng thái:{" "}
                          <strong className={(viewingPollStatsNotif.isPollClosed || viewingPollStatsNotif.pollClosed || viewingPollStatsNotif.pollStatus === "closed") ? "text-rose-600 font-black" : "text-emerald-600 font-black"}>
                            {(viewingPollStatsNotif.isPollClosed || viewingPollStatsNotif.pollClosed || viewingPollStatsNotif.pollStatus === "closed") ? "Đã ngưng (Đã ẩn với thành viên)" : "Đang mở bình chọn"}
                          </strong>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 bg-white p-4 rounded-xl border border-slate-200/80">
                      <h4 className="text-xs font-black text-slate-800 uppercase tracking-wider">Phân tích tỷ lệ bình chọn</h4>
                      <div className="space-y-2.5">
                        {Array.isArray(viewingPollStatsNotif.pollOptions) && viewingPollStatsNotif.pollOptions.map((opt: string, i: number) => {
                          const count = Object.values(statsVotes).filter(
                            (v: any) => v.optionIndex === i || v.optionText === opt,
                          ).length;
                          const pct = totalVotes > 0 ? Math.round((count / totalVotes) * 100) : 0;
                    ...
                    {/* Detailed user votes & feedback comments */}
                    <div className="space-y-3">
                      <h4 className="text-xs font-black text-slate-800 uppercase tracking-wider">
                        Danh sách phản hồi chi tiết ({totalVotes})
                      </h4>

                      {totalVotes === 0 ? (
                        <p className="text-xs text-slate-400 italic py-4 text-center">Chưa có thành viên nào gửi phản hồi.</p>
                      ) : (
                        <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1">
                          {Object.entries(statsVotes).map(([uid, voteData]: [string, any]) => {
                            const voteDate = voteData.votedAt || voteData.timestamp
                              ? new Date(voteData.votedAt || voteData.timestamp).toLocaleString("vi-VN", {
                                  day: "2-digit",
                                  month: "2-digit",
                                  hour: "2-digit",
                                  minute: "2-digit",
                            ...
                            return (
                              <div key={uid} className="p-3 bg-slate-50 rounded-xl border border-slate-200/60 space-y-1.5">
                                <div className="flex items-center justify-between text-xs">
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-bold text-slate-800">{voteData.userName || "Thành viên"}</span>
                                    <span className="text-[10px] text-slate-400 font-mono">({voteData.userEmail || uid})</span>
                                  </div>
                                  <span className="text-[10px] text-slate-400 font-mono">{voteDate}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="px-2 py-0.5 bg-purple-100 text-purple-800 font-bold text-[10px] rounded-md">
                                    Đã chọn: {voteData.optionText}
                                  </span>
                                </div>
                                {voteData.comment && (
                                  <p className="text-xs text-slate-600 bg-white p-2 rounded-lg border border-slate-200 font-medium italic mt-1">
                                    💬 "{voteData.comment}"
                                  </p>
                                )}
                              </div>
            ...
            >
              ...
              >
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="space-y-3">
                    <div className="bg-indigo-50/50 rounded-2xl p-4 border border-indigo-100/50 space-y-3.5">
                      {/* Zalo Info */}
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-0.5 min-w-0 flex-1">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Liên hệ qua Zalo</span>
                          <div className="flex items-center gap-2 flex-wrap">
                            <a
                              href="https://zalo.me/0335784563"
                              target="_blank"
                              rel="noreferrer"
                              className="text-xs md:text-sm font-bold text-blue-600 hover:underline select-all"
                            >
                              0335784563
                            </a>
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-[10px] font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 px-2 py-0.5 rounded-full transition-colors inline-flex items-center gap-1"
                            >
                              Nhắn Zalo ngay ↗
                            </a>
                          </div>
                      ...
                      {/* Email info */}
                      <div className="flex items-start gap-3 pt-1 border-t border-indigo-100/60">
